...
public class ToAccountDao {
	public boolean addAccount(BankAccount account) {
		String query = "Insert into toaccount (account_id,account_name,account_type,account_balance)  values(?,?,?,?) ";
	...
	public BankAccount findAccountById(double id) {
		String query = "Select * from toaccount where account_id=" + id;
	...
	public boolean updateBankAccount(double id, double account) throws SQLException {
		BankAccount account1 = findAccountById(id);
		System.out.println(account);

		String query = "UPDATE toaccount SET account_balance=" + account + " WHERE account_id=" + id;
		try (Connection connection = DBUtil.getConnection();
				PreparedStatement statement = connection.prepareStatement(query)) {
